...
// ── AC: createRandom is a FRESH-per-call entropy-seeded RandomSource factory (not a shared instance) ──
// Each Generate must get an independent deterministic stream — a shared mutable source would make walk N
// depend on every prior draw, breaking US-022's single-seed reproducibility. So the factory returns a
// distinct, fully-functional RandomSource each call.
{
  ...
  ok("createRandom(): a fresh, functional, entropy-seeded RandomSource per call (seed-agnostic factory)");
}
