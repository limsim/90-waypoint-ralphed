...
/**
 * An immutable axis-aligned rectangle in generation-space coordinates.
...
export class Bounds {
  ...
  /**
   * Returns true when the point lies within the bounds inset by `padding` on all sides.
   * At padding=0 the boundary itself is inclusive.
   */
  ...
  /**
   * Returns a new Bounds scaled outward from its centre by `factor`.
   * factor=1 leaves the bounds unchanged; factor=1.1 grows each side by 5%.
   */
